...
// 	$resos=mysqli_query($GLOBALS["___mysqli_ston"],"select * from user_registration where user_id='$sponsor' || username='$sponsor'");
// 	$username_exist = mysqli_num_rows($resos);
// 	if($username_exist==0){
// 	    header("Location:cmsadmin/poc_registration.php?msg=Sponsor Not Exist!");
//     	exit;
// 	}else{
// 	    $row_ref=mysqli_fetch_assoc($resos);
// 	    $sponsorid = $row_ref['user_id'];
// 	}
...
// 	}
  ...
  //  $package = $_POST['package'];
	...
	$resusername=mysqli_query($GLOBALS["___mysqli_ston"],"select * from poc_registration where email='$email'");
	$pocuser_exist = mysqli_num_rows($resusername);
	if($pocuser_exist>0){
	   
	    header("Location:franchisepanel/registration_new.php?msg=Email Id Already Exist!");
    	exit;
	}
	
    $franchise_category=$_POST['franchise_category'];
    if($user_id=='1749318' || $user_id==''){
        $user_id=franchiseid();
	}
        ...
        $longi=$_POST['longi'];
        if($user_id!=''){
            $invoice_no = $user_id.mt_rand(444444,999999);
    //var_dump($_FILES);exit;
    $image = '';
    $storeImage='';
    if(isset($_FILES['upload']['name']) && $_FILES['upload']['name'] != ''){
        $image=time()."WH".$_FILES['upload']['name'];
        $uploads_dir = 'cmsadmin/warehouse_images';
    ...
    if(isset($_FILES['storeimage']['name']) && $_FILES['storeimage']['name'] != ''){
        $storeImage=time()."WH".$_FILES['storeimage']['name'];
        $uploads_dire = '../Vensemart/storage/app/shop_images';
        $tmp_nameone =$_FILES["storeimage"]["tmp_name"];
        move_uploaded_file($tmp_nameone, $uploads_dire."/".$storeImage); 
    }
    
    $insertarrayone=array(
                        'store_name'=>$_POST['fullname'],
     ...
     $mxDb->insert_record('stores', $insertarrayone);
    // $str='insert into banner set created_on="'.date('Y-m-d').'", status="1",image= "'.$image.'"';
    // $res=mysqli_query($GLOBALS["___mysqli_ston"], $str)or die("error");
    
    
            $insert_array = array('country_code'=>$countrycode,'ref_id'=>123456,'user_id'=>$user_id,'username'=>$username,'password'=>$password,'first_name'=>$firstname,'last_name'=>$lastname,'email'=>$email,'telephone'=>$phone,'address'=>$address,
            'city'=>$city,'state'=>$state,'country'=>$country,'lendmark'=>$lendmark,  'zipcode'=>'', 'admin_status'=>0,  'user_status'=>"0",'registration_date'=>$date,     'image'=>$image,  'acc_name'  => '','ac_no' => '','bank_nm' => '','branch_nm' => '','swift_code' => '','last_login_date' => '','current_login_date' => '','id_card' => '','id_no' => '','kyc_status' => '','activation_date' => '',
            'franchise_category'=>$franchise_category,'franchise_satus'=>$franchise_satus,'gst'=>$gst, 'lati'=> $lati, 'longi' =>$longi,'created_at'=> $date, 'updated_at'=>$date,'user_status'=>$userstatus        );
		
		
			if($mxDb->insert_record('poc_registration', $insert_array)){
		        mysqli_query($GLOBALS["___mysqli_ston"], "insert into poc_e_wallet values(NULL,'$user_id','$package','0')");
		        mysqli_query($GLOBALS["___mysqli_ston"], "insert into poc_income_wallet values(NULL,'$user_id','0','0')");
		      //  $sponsorcommission = $package*1/100;
		      //  $tdsper = 5;
		      // $deduction = $sponsorcommission*$tdsper/100;
		      //  $finalcommission = $sponsorcommission-$deduction;
                //mysqli_query($GLOBALS["___mysqli_ston"], "update shopping_e_wallet set amount=(amount+$finalcommission) where user_id='$sponsorid'");
                /*mysqli_query($GLOBALS["___mysqli_ston"], "update final_e_wallet set amount=(amount+$rwallet) where user_id='".$ref."'");*/
              //  $urls="http://".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
    	     //   mysqli_query($GLOBALS["___mysqli_ston"], "insert into credit_debit values(NULL,'$invoice_no','$sponsorid','$finalcommission','0','$deduction','$sponsorid','$user_id','$date','Franchise Direct Income','Franchise Direct Income from $user_id','$finalcommission','Referral Income','$invoice_no','Franchise Direct Income','1','E Wallet',CURRENT_TIMESTAMP,'$urls')");
   	
   	header("Location:franchisepanel/login.php?msg1=Registration completed successfully");
	exit;
			}
			else{
	...
	global $mxDb;
     ...
     if($username!='' && $password!='' )
	 {
		if(strlen($username) && strlen($password))
        {
            ...
            $queryapproval=mysqli_query($GLOBALS["___mysqli_ston"], "select * from poc_registration where ((is_verified='0' && admin_status='0') && ((email = '$username' || username='$username' || user_id='$username') && password = '$password'))");
			$resultapproval=mysqli_fetch_array($queryapproval);

			if($numapproval=mysqli_num_rows($queryapproval))
			{
			    
			    header("location:franchisepanel/login.php?msg=approval");
			}
			else
			{
			    $query=mysqli_query($GLOBALS["___mysqli_ston"], "select * from poc_registration where ((is_verified='1' && admin_status='0') && ((email = '$username' || username='$username' || user_id='$username') && password = '$password'))");
				...
				{ 
					...
					if($url!=''){ ?>		
				 	<script type="text/javascript">window.location.href='franchisepanel/index.php';</script>
			         <?php exit();  }else { ?>
				...
				{
					header("location:franchisepanel/login.php?msg=wrong");
       			}
			}
...
function franchiseid(){
	...
	$encypt1=uniqid(rand(1000000000,9999999999), true);
	$usid1=str_replace(".", "", $encypt1);
	$pre_userid = "WH".substr($usid1, 0, 7);
	$checkid=mysqli_query($GLOBALS["___mysqli_ston"], "select user_id from $table_name where user_id='$pre_userid'");
	if(mysqli_num_rows($checkid)>0){
		franchiseid();
	}else
...
$_SESSION['leg']=$resultings1;
		...
		if($user_id!='' && $_SESSION['sponsorid']!='' && $_SESSION['username']!='' && $_SESSION['group_type']!=''){
			...
			if($mxDb->insert_record('user_registration', $insert_array)){
		    	...
		    	mysqli_query($GLOBALS["___mysqli_ston"], "insert into shopping_e_wallet values(NULL,'$user_id','0','0')");
