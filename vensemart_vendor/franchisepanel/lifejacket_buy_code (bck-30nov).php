...
function commission_of_referal($ref,$useridss,$amount,$invoice_no,$packages)
{
	...
	mysqli_query($GLOBALS["___mysqli_ston"], "insert into credit_debit values(NULL,'$invoice_no','$ref','$rwallet','0','0','$ref','$useridss','$date','Sponsor Income','Earn Sponsor Income from $useridss for $packages Package','Commission of USD $rwallet For Package ".$packages." ','Sponsor Income','$invoice_no','Sponsor Income','1','Withdrawal Wallet',CURRENT_TIMESTAMP,'$urls')");	
	}
}
...
if(($_POST['package_id']!='') && ($_POST['password']!='') && ($_POST['amount']!=''))
{
   ...
   mysqli_query($GLOBALS["___mysqli_ston"], "update user_registration set issued='UPG', product_type='NA' where user_id='$userid'");
    ...
    commission_of_referal($ref['ref_id'],$userid,$amount,$invoice_no,$_POST['package_id']);
