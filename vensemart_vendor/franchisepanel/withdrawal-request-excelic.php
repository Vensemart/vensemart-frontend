...
header("Expires: 0");
		   if(isset($_POST['Show']))
		   {
			  ...
			  $title = '';$title .= "First Name,Last Name,Account Name ,Account Number,Bank Name,Branch Name,Swift Code,Amount to pay,Transaction Charge,Request Amount ,BTC Address,Pay Mode"."\n";
echo $title;
			if(!empty($id)) {
    foreach($id as $check)   {
			  
			$select=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from 	withdraw_request_cp where id='$check'"));
...
$sql = "SELECT * FROM 	withdraw_request_cp where id='$check' and status='0'";
		 ...
		 $content .= escape_csv_value($row['swift_code']).",";
		  $content .= escape_csv_value($row['request_amount']).",";
		   $content .= escape_csv_value($row['transaction_charge']).",";
		    ...
		    $content .= escape_csv_value($row['btc_address']).",";
		    $content .= escape_csv_value($row['pay_mode']).",";
...
echo $content;
mysqli_query($GLOBALS["___mysqli_ston"], "update 	withdraw_request_cp set status='1', admin_remark='$description', admin_response_date='$date' where id='$check'");
