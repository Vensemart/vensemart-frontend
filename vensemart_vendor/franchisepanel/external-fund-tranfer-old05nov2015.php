      ...
      if( $_POST['t_password'] == $f['t_code'] )
      {
        ...
        if( $args_wallet['amount'] >= $total_charge)
        {
          ...
          if( mysqli_num_rows($rst_receiver) > 0 ){
            ...
            $sql_transfer .= "Remark='Transfer fund ".$_POST['amounts']." USD to ".$args_receiver['first_name']." ".$args_receiver['last_name']." with ".$admin_charge."% admin charge',";
            ...
            //exit;
            echo "<script language='javascript'> alert('Fund transfer successfully!'); window.location.href='external-fund-tranfer.php'; </script>";
          }
          else{
          ...
          <div class="row">
           ...
           <form name="bankinfo" method="post">
              <section class="panel">
                ...
                <header class="panel-heading">
                 <br/> <h3 class="panel-title">e-Wallet Ballance :  <?php $data=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from final_e_wallet where user_id='$userid'"));?><strong><?php echo $data['amount'];?></strong> SGD</h3>
                <br/></header>
...
<input name="wallet" id="wallet" type="hidden" tabindex="1" required class="" style="width:4%;" value="final_e_wallet" checked="checked" />
           ...
           <div class="form-group">
                      <label for="exampleInputAddress">Enter sender userid / username</label>
