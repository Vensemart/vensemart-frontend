...
{
   echo "<script language='javascript'> alert('Enter Details!'); window.location.href='external-fund-tranfer-p.php'; </script>";
}
...
{
   echo "<script language='javascript'> alert('Enter Numbers only!'); window.location.href='external-fund-tranfer-p.php'; </script>";
}
 ...
 if( $_POST['t_password'] != $f['password'] )
      {
          ...
          echo "<script language='javascript'> alert('Wrong Password!'); window.location.href='external-fund-tranfer-p.php'; </script>";
        }
 ...
 if( $roesding['user_id'] == $f['user_id'] )
      {
          ...
          echo "<script language='javascript'> alert('You Cannot transfer to your own account!'); window.location.href='external-fund-tranfer-p.php'; </script>";
        }
...
if($roesd==0)
      {
          echo "<script language='javascript'> alert('Sorry no such member found!'); window.location.href='external-fund-tranfer-p.php'; </script>";
      }
      else
      {
        $roesding=mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "select * from user_registration where user_id='".$_POST['user']."' || username='".$_POST['user']."'"));
        ...
        // else
        // {
        //       echo "<script language='javascript'> alert('User not found in your downline team!'); window.location.href='external-fund-tranfer-p.php'; </script>";
        // }
...
<?php include("includes/tittle.php");?>
  ...
  <form name="bankinfo" method="post" action="external-fund-transfer-code-p.php">
              <section class="panel">

                <header class="panel-heading">
                  <h3 class="panel-title">System Wallet fund transfer confirmation</h3>
                </header>
               
                <div class="panel-body">
<input name="wallet" id="wallet" type="hidden" tabindex="1" required class="" style="width:4%;" value="balance_e_wallet" checked="checked" />
