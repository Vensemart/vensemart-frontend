...
if (isset($_POST['pancard'])) 
    ...
    if (isset($_POST['address'])) 
 {
    ...
    $h_addid=$_POST['hidden_add_id'];
     ...
     {
          move_uploaded_file($tmp_name,$folder.$date.$pic);
   mysqli_query($GLOBALS["___mysqli_ston"], "update address_proof_list set user_id='$uid',address_proof='$add_proof',address_proof_image='$imageDBpath',id_checkbox='$check_yes',status='0' where id='$h_addid'");
     }
     else {
          move_uploaded_file($tmp_name,$folder.$date.$pic);
    mysqli_query($GLOBALS["___mysqli_ston"], "insert into address_proof_list (user_id,address_proof,address_proof_image,id_checkbox,status) values ('$uid','$add_proof','$imageDBpath','$check_yes','0')");
